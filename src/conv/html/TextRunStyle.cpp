...
void ListStyleManager::List::setLevel(int lvl, librevenge::RVNGPropertyList const &pList, bool ordered)
{
	...
	else
	{
		...
		if (pList["style:num-format"])
		{
			...
			{
				MWAW_DEBUG_MSG(("ListStyleManager::List::setLevel: unknown format %s\n",
...
std::string ListStyleManager::List::str() const
{
	...
	{
		MWAW_DEBUG_MSG(("ListStyleManager::List::str: level %d is not set\n", m_level));
		return "";
	}
...
void ListStyleManager::closeLevel()
{
	...
	{
		MWAW_DEBUG_MSG(("ListStyleManager::closeLevel: can not find level\n"));
		return;
	}
...
std::string ParagraphStyleManager::getContent(librevenge::RVNGPropertyList const &pList, bool isList) const
{
	...
	if (isList)
	{
		...
		if (pList["fo:margin-left"]
		        && mwaw2html::getPointValue(*pList["fo:margin-left"], tmp))
		{
			val += tmp;
		}
		if (pList["fo:text-indent"]
		        && mwaw2html::getPointValue(*pList["fo:text-indent"], tmp))
		{
			val += tmp;
		}
...
void SpanStyleManager::parseTextPosition(char const *value, std::ostream &out) const
{
	...
	{
		MWAW_DEBUG_MSG(("SpanStyleManager::parseTextPosition: can not read percent: find %s\n", s.str().c_str()));
		percent=100;
	}
