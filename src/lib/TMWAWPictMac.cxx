...
{
PictMac::ReadResult PictMac::checkOrGet
(TMWAWInputStreamPtr input, int size, Box2f &box, PictData **result)
{
  if (result) *result=0L;
  ...
  libmwaw_tools::PictMac *res = new PictMac(box);
 ...
 */
namespace libmwaw_tools_mac_pict1
{
...
public:
  ...
  bool read(TMWAWInputStream &input) {
    long actualPos = input.tell();
...
struct Bitmap {
  ...
  bool read (TMWAWInputStream &input, bool packed, bool hasRegion) {
  ...
  bool saveBitmap() const {
    ...
    PictBitmapBW bitmap(Vec2i(m_rect.size().x(),nRows));
    ...
    return Debug::dumpFile(dt, f.str().c_str());
  }
  ...
  bool readBitmapData(TMWAWInputStream &input, bool packed) {
...
struct Pixmap {
  ...
  bool read (TMWAWInputStream &input, bool packed, bool colorTable, bool hasRectsMode, bool hasRegion) {
  ...
  bool savePixmap() const {
    ...
    if (m_colorTable.get() && m_indices.size()) {
      ...
      PictBitmapIndexed pixmap(Vec2i(W,nRows));
      ...
      PictBitmapColor pixmap(Vec2i(W,nRows));
      if (!pixmap.valid()) return false;
  ...
  bool readPixmapData(TMWAWInputStream &input) {
...
struct OpCode {
  ...
  bool readData(TMWAWInputStream &input, std::vector<Value> &listValue) const {
  ...
  bool computeSize(TMWAWInputStream &input, int &sz) const {
    long actPos = input.tell();
    sz = 0;
  ...
  static bool readRect(TMWAWInputStream &input, DataType type, Box2i &res) {
    Vec2i v[2];
    DataType valType;
    switch(type) {
    case WP_RECT:
...
protected:
  ...
  static int getSize(TMWAWInputStream &input, DataType type) {
    switch(type) {
    case WP_BYTE:
    case WP_UBYTE:
      return 1;
  ...
  static bool readValue(TMWAWInputStream &input, DataType type, Value &val) {
    switch(type) {
    case WP_BYTE:
    case WP_UBYTE:
    case WP_INT:
  ...
  static bool readInt(TMWAWInputStream &input, DataType type, int &res) {
    int sz = 0;
    long actualPos = input.tell();
    res = 0;
    switch(type) {
  ...
  static bool readColor(TMWAWInputStream &input, DataType type, int (&col)[3]) {
    if (type != WP_COLOR) {
   ...
   *
   * \note check if this is not an indexed color */
  static bool readCColor(TMWAWInputStream &input, DataType type, int (&col)[3]) {
    if (type != WP_CCOLOR) {
  ...
  static bool readPattern(TMWAWInputStream &input, DataType type, int (&pat)[8]) {
    if (type != WP_PATTERN) {
  ...
  static bool readPoint(TMWAWInputStream &input, DataType type, Vec2i &res) {
    int v[2];
    DataType valType;
    switch(type) {
    case WP_POINT:
  ...
  static bool readPoly(TMWAWInputStream &input, DataType type, Box2i &box, std::vector<Vec2i> &res) {
  ...
  static bool readText(TMWAWInputStream &input, DataType type, std::string &res) {
    int sz = 0;
    switch (type) {
    case WP_TEXT:
...
public:
   ...
   * in another format
   */
  void parse(TMWAWInputStreamPtr input, libmwaw_tools::DebugFile &dFile);
 ...
 * in another format
 */
void PictParser::parse(TMWAWInputStreamPtr input, libmwaw_tools::DebugFile &dFile)
{
  libmwaw_tools::DebugStream s;
  long actPos = 0L;
  bool ok = true;
...
#  define ADD_DATA_SHORT(resPtr,val) do {			\
  ...
  *(resPtr++) = (unsigned char) (val & 0xFF); } while(0)
  ...
  TMWAWInputStream input(const_cast<WPXInputStream *>(orig.getDataStream()), false);
 ...
 */
namespace libmwaw_tools_mac_pict2
{
using namespace libmwaw_tools_mac_pict1;

//! Internal and low level: a class used to read and store all possible value
struct Value : public libmwaw_tools_mac_pict1::Value {
};

//! Internal and low level: a class to define each opcode and their arguments and read their data
struct OpCode : public libmwaw_tools_mac_pict1::OpCode {
  typedef libmwaw_tools_mac_pict1::OpCode parent;
  /** constructor
   *
   * \param id is the code of the opcode in the file
   * \param nm is the short name of the opcode
...
public:
  //! the constructor
  PictParser() : m_mapIdOp() {
    int numCodes = sizeof(libmwaw_tools_mac_pict1::s_listCodes)/sizeof(OpCode);
    for (int i = 0; i < numCodes; i++)
      m_mapIdOp[libmwaw_tools_mac_pict1::s_listCodes[i].m_id] = &(s_listCodes[i]);
...
void PictMac::parsePict1(WPXBinaryData const &pict, std::string const &fname)
{
  TMWAWInputStream *input = new TMWAWInputStream(const_cast<WPXInputStream *>(pict.getDataStream()), false);
  TMWAWInputStreamPtr ip(input);

  DebugFile dFile(ip);
  dFile.open(fname);
  libmwaw_tools_mac_pict1::s_parser.parse(ip, dFile);
}

void PictMac::parsePict2(WPXBinaryData const &pict, std::string const &fname)
{
  TMWAWInputStream *input = new TMWAWInputStream(const_cast<WPXInputStream *>(pict.getDataStream()), false);
  TMWAWInputStreamPtr ip(input);
  ...
  DebugFile dFile(ip);
  dFile.open(fname);
  libmwaw_tools_mac_pict2::s_parser.parse(ip, dFile);
}

bool PictMac::convertPict1To2(WPXBinaryData const &orig, WPXBinaryData &result)
{
  static bool volatile conversion = false;
  while (conversion) ;
  conversion = true;
  bool ok = libmwaw_tools_mac_pict1::s_parser.convertToPict2(orig, result);
  conversion = false;
  if (!ok) return false;
  ...
  if (1) {
    ...
    Debug::dumpFile(const_cast<WPXBinaryData &>(orig), f.str().c_str());
