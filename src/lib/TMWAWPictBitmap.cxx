...
template <class T>
bool getPBMData(PictBitmapContainer<T> const &orig, WPXBinaryData &data, T white)
{
  Vec2i sz = orig.size();
...
template <class T>
bool getPPMData(PictBitmapContainer<T> const &orig, WPXBinaryData &data, std::vector<Vec3uc> const &indexedColor)
{
  Vec2i sz = orig.size();
  if (sz[0] <= 0 || sz[1] <= 0) return false;
  ...
  for (int j = 0; j < sz[1]; j++) {
    ...
    for (int i = 0; i < sz[0]; i++) {
      ...
      if (ind < 0 || ind >= nColors) {
        MWAW_DEBUG_MSG(("PictBitmap::getPPMData invalid index %d\n", ind));
        return false;
      }
...
bool getPPMData(PictBitmapContainer<Vec3uc> const &orig, WPXBinaryData &data)
{
  Vec2i sz = orig.size();
  if (sz[0] <= 0 || sz[1] <= 0) return false;
...
bool PictBitmapBW::createFileData(WPXBinaryData &result) const
{
...
bool PictBitmapColor::createFileData(WPXBinaryData &result) const
{
...
bool PictBitmapIndexed::createFileData(WPXBinaryData &result) const
{
