...
template <class T> class PictBitmapContainer
{
public:
  //! constructor given size
  PictBitmapContainer(Vec2i const &size) : m_size(size), m_data(0L) {
    if (m_size[0]*m_size[1] != 0) m_data = new T[m_size[0]*m_size[1]];
  }
  //! destructor
  virtual ~PictBitmapContainer() {
...
private:
  PictBitmapContainer(PictBitmapContainer const &orig);
  PictBitmapContainer &operator=(PictBitmapContainer const &orig);
protected:
  //! the size
  Vec2i m_size;
...
class PictBitmapContainerBool : public PictBitmapContainer<bool>
{
public:
  //! constructor
  PictBitmapContainerBool(Vec2i const &size) : PictBitmapContainer<bool>(size) {}
...
public:
  ...
  virtual int cmp(Pict const &a) const {
    int diff = Pict::cmp(a);
    if (diff) return diff;
    PictBitmap const &aPict = static_cast<PictBitmap const &>(a);
...
public:
  ...
  must be redefined in the subs class */
  virtual int cmp(Pict const &a) const {
    int diff = PictBitmap::cmp(a);
    if (diff) return diff;
    PictBitmapBW const &aPict = static_cast<PictBitmapBW const &>(a);
  ...
  //! the constructor
  PictBitmapBW(Vec2i const &sz) : PictBitmap(sz), m_data(sz) { }
...
protected:
  ...
  //! the data
  PictBitmapContainerBool m_data;
};

/** a bitmap of int to store indexed bitmap */
class PictBitmapIndexed : public PictBitmap
{
public:
  ...
  virtual int cmp(Pict const &a) const {
    int diff = PictBitmap::cmp(a);
    if (diff) return diff;
    PictBitmapIndexed const &aPict = static_cast<PictBitmapIndexed const &>(a);
  ...
  //! the constructor
  PictBitmapIndexed(Vec2i const &sz) : PictBitmap(sz), m_data(sz), m_colors() { }
...
public:
  ...
  virtual int cmp(Pict const &a) const {
    int diff = PictBitmap::cmp(a);
    if (diff) return diff;
    PictBitmapColor const &aPict = static_cast<PictBitmapColor const &>(a);
  ...
  //! the constructor
  PictBitmapColor(Vec2i const &sz) : PictBitmap(sz), m_data(sz) { }
...
protected:
  ...
  //! the data
  PictBitmapContainer<Vec3uc> m_data;
};
}
#endif
