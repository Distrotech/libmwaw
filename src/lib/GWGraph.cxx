...
bool GWGraph::readGraphicZone()
{
  ...
  if (!isGraphicZone() && !findGraphicZone()) {
    input->seek(beginPos, RVNG_SEEK_SET);
    return false;
  }
  long pos = input->tell();
...
bool GWGraph::findGraphicZone()
{
  ...
  while(!input->atEOS()) {
    ...
    if (value==0x20FFFF)
      ...
      decal = 3;
    if (decal>=0) {
      input->seek(actPos-decal, RVNG_SEEK_SET);
    ...
    input->seek(actPos-decal-pageHeaderSize-headerSize, RVNG_SEEK_SET);
    if (!isGraphicZone()) {
      input->seek(actPos+4, RVNG_SEEK_SET);
      continue;
    }
...
bool GWGraph::isPageFrames()
{
  ...
  input->seek(pos, RVNG_SEEK_SET);
  if (hasPageUnknown) {
    input->seek(2, RVNG_SEEK_CUR); // page
  ...
  for (int i=0; i < nZones; ++i) {
    ...
    input->seek(expectedSz[i]*nData, RVNG_SEEK_CUR);
  }
  input->seek(pos, RVNG_SEEK_SET);
  return true;
}
...
bool GWGraph::readStyle(MWAWGraphicStyle &style)
{
  ...
  input->seek(pos+116, RVNG_SEEK_SET);
  libmwaw::DebugFile &ascFile = m_parserState->m_asciiFile;
  ...
  input->seek(2, RVNG_SEEK_CUR); // junk ?
  val=(int) input->readLong(2);
  switch(val) {
  case 1: // none
    break;
...
bool GWGraph::readLineFormat(std::string &extra)
{
  ...
  input->seek(pos+gDataSize, RVNG_SEEK_SET);
  return true;
}
...
bool GWGraph::readPageFrames()
{
  ...
  for (int i=0; i < nFrames; ++i) {
    ...
    if (!zone) {
      ...
      f << "###";
      input->seek(pos+0x36, RVNG_SEEK_SET);
  ...
  if (vers==1) {
    ...
    for (int i=0; i < nData; ++i) {
      ...
      ascFile.addPos(pos);
      ascFile.addNote(f.str().c_str());
      input->seek(pos+gLineFSize, RVNG_SEEK_SET);
    }
    pos=input->tell();
    if (pos!=zoneEnd) {
      ascFile.addPos(pos);
  ...
  for (int id=nFrames-1; id >= 0; --id) {
    ...
    if (zone->m_dataSize>0 && input->tell()!=pos+zone->m_dataSize) {
      ...
      input->seek(pos+zone->m_dataSize, RVNG_SEEK_SET);
      ok = true;
    }
    if (ok)
      continue;
...
bool GWGraph::readFrameExtraDataRec(GWGraphInternal::Zone &zone, int id, std::set<int> &seens, long endPos)
{
  ...
  if (frame->m_dataSize>0 && input->tell()!=pos+frame->m_dataSize) {
    ...
    input->seek(pos+frame->m_dataSize, RVNG_SEEK_SET);
  }
  if (frame->getType()!=GWGraphInternal::Frame::T_GROUP)
    return true;
  GWGraphInternal::FrameGroup &group=reinterpret_cast<GWGraphInternal::FrameGroup &>(*frame);
...
bool GWGraph::readFrameExtraData(GWGraphInternal::Frame &frame, int id, long endPos)
{
  ...
  case 1: {
    ...
    input->seek(pos+text.m_dataSize, RVNG_SEEK_SET);
    ascFile.addPos(pos);
    ascFile.addNote(f.str().c_str());
    return true;
  }
  ...
  case 11: {
    ...
    input->seek(pos+pict.m_dataSize, RVNG_SEEK_SET);
    return true;
  }
  case 15: {
    ...
    if (frame.getType()!=GWGraphInternal::Frame::T_GROUP) {
      ...
      input->seek(pos+4+2*nGrp, RVNG_SEEK_SET);
      f << "###[internal]";
      return false;
    }
    GWGraphInternal::FrameGroup &group=reinterpret_cast<GWGraphInternal::FrameGroup &>(frame);
...
bool GWGraph::sendFrame(shared_ptr<GWGraphInternal::Frame> frame, GWGraphInternal::Zone const &zone)
{
  ...
  MWAWPosition fPos(frame->m_box[0]+LTPos,frame->m_box.size(),RVNG_POINT);
