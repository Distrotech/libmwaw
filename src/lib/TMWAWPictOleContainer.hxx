...
class PictOleContainer : public Pict
{
public:
  //! returns the picture type
  virtual Type getType() const {
    return Pict::OleContainer;
  }
  ...
  static PictOleContainer *get(TMWAWInputStreamPtr input, int size) {
    PictOleContainer *res = 0L;
    Box2f box;
  ...
  virtual int cmp(Pict const &a) const {
    int diff = Pict::cmp(a);
    if (diff) return diff;
    PictOleContainer const &aPict = static_cast<PictOleContainer const &>(a);
...
protected:
  ...
  PictOleContainer(Box2f ) : m_data(), m_empty(false) { }
  PictOleContainer( ) : m_data(), m_empty(false) { }
  ...
  static ReadResult checkOrGet(TMWAWInputStreamPtr input, int size,
                               Box2f &box, PictOleContainer **result = 0L);
