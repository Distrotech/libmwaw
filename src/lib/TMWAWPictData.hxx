...
public:
  ...
  static ReadResult check(TMWAWInputStreamPtr input, int size, Box2f &box) {
  ...
  static PictData *get(TMWAWInputStreamPtr input, int size) {
    PictData *res = 0L;
    Box2f box;
  ...
  virtual int cmp(Pict const &a) const {
    int diff = Pict::cmp(a);
    if (diff) return diff;
    PictData const &aPict = static_cast<PictData const &>(a);
...
protected:
  ...
  PictData(): m_data(), m_empty(false) { }
  PictData(Box2f &): m_data(), m_empty(false) { }
...
protected:
   ...
   *    - creates a picture if result is given and if the picture is not empty */
  static ReadResult checkOrGet(TMWAWInputStreamPtr input, int size, PictData **result = 0L);
};

//! class to store small data which are potentially a picture
class PictDUnknown : public PictData
{
public:
  //! returns the picture subtype
  virtual SubType getSubType() const {
