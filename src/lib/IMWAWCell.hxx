...
public:
   ...
   */
  //! constructor
  IMWAWCellFormat() : m_format(F_UNKNOWN), m_subFormat(0), m_digits(0),
    m_hAlign(HALIGN_DEFAULT), m_bordersList(0), m_protected(false) { }

  virtual ~IMWAWCellFormat() {}
  ...
  int compare(IMWAWCellFormat const &cell) const;

  //! operator<<
  friend std::ostream &operator<<(std::ostream &o, IMWAWCellFormat const &cell);
...
public:
  ...
  //! the constructor
  IMWAWCellContent() : m_contentType(C_UNKNOWN), m_value(0.0), m_valueSet(false),
    m_textValue(""), m_textValueSet(false), m_formulaValue("") { }
  virtual ~IMWAWCellContent() {}
   ...
   */
  bool getDataCellProperty(IMWAWCellFormat::Format format, WPXPropertyList &property,
  ...
  //! operator<<
  friend std::ostream &operator<<(std::ostream &o, IMWAWCellContent const &cell);
...
class IMWAWCell : public IMWAWCellFormat
{
public:
  //! constructor
  IMWAWCell() : m_position(0,0), m_numberCellSpanned(1,1) {}
