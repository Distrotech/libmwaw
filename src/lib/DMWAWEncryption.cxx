...
DMWAWEncryption::DMWAWEncryption(const char *password, const unsigned long encryptionStartOffset) :
  m_buffer(NULL),
  m_password(),
...
DMWAWEncryption::~DMWAWEncryption()
{
  if (m_buffer)
    delete [] m_buffer;
}
...
uint16_t DMWAWEncryption::getCheckSum() const
{
  if (m_password.len() <= 0)
    return 0;
...
const unsigned char *DMWAWEncryption::readAndDecrypt(WPXInputStream *input, unsigned long numBytes, unsigned long &numBytesRead)
{
