...
#ifndef MWAW_ASCII_DEB
#  define MWAW_ASCII_DEB
...
public:
  ...
  DebugFile(TMWAWInputStreamPtr ip=TMWAWInputStreamPtr())
    : m_file(), m_on(false), m_input(ip), m_actOffset(-1), m_notes(), m_skipZones() { }

  //! resets the input
  void setStream(TMWAWInputStreamPtr ip) {
    m_input = ip;
  }
  //! destructor
  ~DebugFile() {
...
{
public:
  DebugFile(TMWAWInputStreamPtr) {}
  DebugFile() {}
  void setStream(TMWAWInputStreamPtr) {  }
  ~DebugFile() { }
