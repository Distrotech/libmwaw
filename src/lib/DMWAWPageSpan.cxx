...
const double DMWAW_DEFAULT_PAGE_MARGIN_TOP = 1.0;
const double DMWAW_DEFAULT_PAGE_MARGIN_BOTTOM = 1.0;
...
DMWAWHeaderFooter::DMWAWHeaderFooter(const DMWAWHeaderFooter &headerFooter) :
...
DMWAWHeaderFooter &DMWAWHeaderFooter::operator=(const DMWAWHeaderFooter &headerFooter)
{
  m_type = headerFooter.getType();
  m_occurence = headerFooter.getOccurence();
  m_internalType = headerFooter.getInternalType();
...
DMWAWHeaderFooter::~DMWAWHeaderFooter()
{
//	delete m_subDocument;
}

DMWAWPageSpan::DMWAWPageSpan() :
  ...
  m_marginLeft(1.0),
  m_marginRight(1.0),
  m_marginTop(DMWAW_DEFAULT_PAGE_MARGIN_TOP),
  m_marginBottom(DMWAW_DEFAULT_PAGE_MARGIN_BOTTOM),
  ...
  m_pageSpan(1)
{
  for (int i=0; i<DMWAW_NUM_HEADER_FOOTER_TYPES; i++)
    m_isHeaderFooterSuppressed[i]=false;
}

DMWAWPageSpan::DMWAWPageSpan(const DMWAWPageSpan &page) :
...
{
  for (uint8_t i=0; i<DMWAW_NUM_HEADER_FOOTER_TYPES; i++)
...
DMWAWPageSpan::DMWAWPageSpan(const DMWAWPageSpan &page, double paragraphMarginLeft, double paragraphMarginRight) :
  m_isPageNumberSuppressed(false),
  m_formLength(page.getFormLength()),
  m_formWidth(page.getFormWidth()),
  m_formOrientation(page.getFormOrientation()),
                                    ...
                                    const  MWAWSubDocument *subDocument)
{
  ...
  if (containsHFLeft && !containsHFRight) {
    ...
    DMWAWHeaderFooter dummyHeader(type, EVEN, DUMMY_INTERNAL_HEADER_FOOTER, 0);
    m_headerFooterList.push_back(dummyHeader);
  } else if (!containsHFLeft && containsHFRight) {
    WPD_DEBUG_MSG(("Inserting dummy header left\n"));
    DMWAWHeaderFooter dummyHeader(type, ODD, DUMMY_INTERNAL_HEADER_FOOTER, 0);
    m_headerFooterList.push_back(dummyHeader);
  }
}

void DMWAWPageSpan::_removeHeaderFooter(DMWAWHeaderFooterType type, DMWAWHeaderFooterOccurence occurence)
{
  for (std::vector<DMWAWHeaderFooter>::iterator iter = m_headerFooterList.begin(); iter != m_headerFooterList.end(); iter++) {
...
bool DMWAWPageSpan::_containsHeaderFooter(DMWAWHeaderFooterType type, DMWAWHeaderFooterOccurence occurence)
{
  for (std::vector<DMWAWHeaderFooter>::iterator iter = m_headerFooterList.begin(); iter != m_headerFooterList.end(); iter++) {
    if ((*iter).getType()==type && (*iter).getOccurence()==occurence)
      return true;
...
inline bool operator==(const DMWAWHeaderFooter &headerFooter1, const DMWAWHeaderFooter &headerFooter2)
{
...
bool operator==(const DMWAWPageSpan &page1, const DMWAWPageSpan &page2)
{
  ...
  const std::vector<DMWAWHeaderFooter> headerFooterList1 = page1.getHeaderFooterList();
  const std::vector<DMWAWHeaderFooter> headerFooterList2 = page2.getHeaderFooterList();
  std::vector<DMWAWHeaderFooter>::const_iterator iter1;
  std::vector<DMWAWHeaderFooter>::const_iterator iter2;
  ...
  WPD_DEBUG_MSG(("WordPerfect: DMWAWPageSpan == comparison finished, found no differences\n"));
