...
int IMWAWCellFormat::compare(IMWAWCellFormat const &cell) const
{
...
std::ostream &operator<<(std::ostream &o, IMWAWCellFormat const &cell)
{
  int subForm = cell.m_subFormat;
  switch(cell.m_format) {
  case IMWAWCellFormat::F_TEXT:
    o << "text";
    if (subForm) {
      o << "[Fo" << subForm << "]";
      subForm = 0;
  ...
  case IMWAWCellFormat::HALIGN_LEFT:
    o << ",left";
    break;
  case IMWAWCellFormat::HALIGN_CENTER:
    o << ",centered";
    break;
  case IMWAWCellFormat::HALIGN_RIGHT:
    o << ",right";
    break;
  case IMWAWCellFormat::HALIGN_FULL:
    o << ",full";
    break;
  default:
    break; // default
...
////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////
bool IMWAWCellContent::getDataCellProperty(IMWAWCellFormat::Format format, WPXPropertyList &propList,
    std::string &text) const
{
  ...
  if (content() == IMWAWCellContent::C_FORMULA && formula().length()) {
    f.str("");
    f << "=" << formula();
  ...
  std::string actualText = IMWAWCellContent::text();
  switch (format) {
  case IMWAWCellFormat::F_TEXT:
  case IMWAWCellFormat::F_NUMBER:
  ...
  case IMWAWCellFormat::F_DATE: {
    int Y=0, M=0, D=0;
    bool dateSet = false;
    if (isValueSet() && IMWAWCellContent::double2Date(value(), Y, M, D)) {
      dateSet = true;
      f.str("");
      f << std::setfill('0');
  ...
  case IMWAWCellFormat::F_TIME: {
    bool tFind = false;
    int H=0, M=0, S=0;
    if (isValueSet() && IMWAWCellContent::double2Time(value(),H,M,S)) {
      tFind = true;
      f.str("");
      f << std::setfill('0');
...
bool IMWAWCellContent::double2Date(double val, int &Y, int &M, int &D)
{
...
bool IMWAWCellContent::double2Time(double val, int &H, int &M, int &S)
{
...
std::ostream &operator<<(std::ostream &o, IMWAWCellContent const &cell)
{
  switch(cell.content()) {
  case IMWAWCellContent::C_NONE:
    break;
  case IMWAWCellContent::C_TEXT:
    o << ",text=\"" << cell.text() << "\"";
    break;
  case IMWAWCellContent::C_NUMBER: {
    o << ",val=";
  ...
  break;
  case IMWAWCellContent::C_FORMULA:
...
////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////
std::ostream &operator<<(std::ostream &o, IMWAWCell const &cell)
{
  o << IMWAWCell::getCellName(cell.m_position, Vec2b(false,false)) << ":";
  if (cell.numSpannedCells()[0] != 1 || cell.numSpannedCells()[1] != 1)
    o << "span=[" << cell.numSpannedCells()[0] << "," << cell.numSpannedCells()[1] << "],";
  o << static_cast<IMWAWCellFormat const &>(cell);

  return o;
}

std::string IMWAWCell::getColumnName(int col)
{
  std::stringstream f;
  f << "[.";
...
std::string IMWAWCell::getCellName(Vec2i const &pos, Vec2b const &absolute)
{
  std::stringstream f;
  f << "[.";
  if (absolute[1]) f << "$";
