...
public:
  //! constructor
  TMWAWPosition(Vec2f const &origin=Vec2f(), Vec2f const &size=Vec2f(), WPXUnit unit=WPX_INCH):
    m_anchorTo(Char), m_xPos(XLeft), m_yPos(YTop), m_wrapping(WNone),
    m_page(0), m_orig(origin), m_size(size), m_naturalSize(), m_unit(unit), m_order(0), m_id(-1) {}

  virtual ~TMWAWPosition() {}
  //! operator<<
  friend  std::ostream &operator<<(std::ostream &o, TMWAWPosition const &pos) {
  ...
  //! basic operator==
  bool operator==(TMWAWPosition const &f) const {
    return cmp(f) == 0;
  }
  //! basic operator!=
  bool operator!=(TMWAWPosition const &f) const {
  ...
  float getInvUnitScale(WPXUnit unit) const {
    ...
    case WPX_INCH:
      ...
      break;
    default:
      MWAW_DEBUG_MSG(("TMWAWPosition::getScaleFor %d unit must not appear\n", int(m_unit)));
    }
    switch(unit) {
    case WPX_TWIP:
      break;
  ...
  void setPage(int page) const {
    const_cast<TMWAWPosition *>(this)->m_page = page;
  }
  ...
  void setPagePos(int page, Vec2f const &newOrig) const {
    const_cast<TMWAWPosition *>(this)->m_page = page;
    const_cast<TMWAWPosition *>(this)->m_orig = newOrig;
  }
  ...
  //! set id
  void setId(long nId) const {
    const_cast<TMWAWPosition *>(this)->m_id = nId;
  }
  //! returns id
  long getId() const {
    return m_id;
...
protected:
  ...
  int cmp(TMWAWPosition const &f) const {
    ...
    long diffI = m_id - f.m_id;
    if (diffI) return diffI < 0 ? -1 : 1;
