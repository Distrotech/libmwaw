...
bool GWText::findNextZone()
{
  ...
  while (true) {
    ...
    if (val==0x20FFFF)
      ...
      input->seek(pos-3, RVNG_SEEK_SET);
    else
      continue;
  ...
  while(true) {
    ...
    if (pos-hSize < searchPos)
      break;
    input->seek(pos-hSize, RVNG_SEEK_SET);
    if (input->readLong(4))
      continue;
    int val=(int)input->readULong(2);
    if (val & 0xFEFE)
  ...
  input->seek(searchPos, RVNG_SEEK_SET);
  return false;
}
...
bool GWText::readZone(GWTextInternal::Zone &zone)
{
  ...
  for (int i=0; i<zone.m_numFonts; ++i) {
    ...
    if (!readFont(font)) {
      f << "###";
      font = MWAWFont();
      input->seek(pos+22, RVNG_SEEK_SET);
    }
  ...
  for (int i=0; i<zone.m_numRulers; ++i) {
    ...
    if (!readRuler(para)) {
      f << "###";
      para = MWAWParagraph();
      input->seek(pos+192, RVNG_SEEK_SET);
    }
...
bool GWText::readRuler(MWAWParagraph &para)
{
  ...
  case 4:
  case 5:
    para.setInterline(spacing[0], RVNG_POINT);
    break;
  case 6:
    para.setInterline(spacing[0], RVNG_PERCENT);
    break;
  default:
...
bool GWText::sendZone(GWTextInternal::Zone const &zone, bool inGraphic)
{
  ...
  while (1) {
    ...
    case 0x4: {
      ...
      MWAWPosition pictPos(Vec2f(0,0), token.m_dim, RVNG_POINT);
