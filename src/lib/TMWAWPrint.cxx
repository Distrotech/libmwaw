...
//
// PrintRect : a rectangle
//
bool PrintRect::read(TMWAWInputStreamPtr input, Vec2i const &res)
{
  int x,y;
...
struct TPrInfo {
  TPrInfo() : m_rect(), m_resolution(), m_iDev(-1) {}
  //! page dimension
  ...
  //! operator <<
  friend std::ostream &operator<< (std::ostream &o, TPrInfo const & r) {
...
struct TPrStl {
  //! operator << \note print nothing
  friend std::ostream &operator<< (std::ostream &o, TPrStl const & ) {
    return o;
  }
...
struct TPrJob {
  //! operator<<
  friend std::ostream &operator<< (std::ostream &o, TPrJob const &r ) {
...
//
// PrintInfo storage class
//
struct PrintInfoImpl {
  PrintInfoImpl(): m_info(), m_paper(), m_feed(), m_info2(), m_job(), m_version(-1) {}
...
//
// PrintInfo : ie. apple TPrint
//
PrintInfo::PrintInfo() : m_data(new PrintInfoImpl) {}
PrintInfo::~PrintInfo()
{
  delete m_data;
}
PrintInfo &PrintInfo::operator=(PrintInfo const &)
{
  MWAW_DEBUG_MSG(("PrintInfo::operator: MUST NOT BE CALLED\n"));
  return *this;
}

PrintRect PrintInfo::page() const
{
  return m_data->m_info.page();
}
PrintRect PrintInfo::paper() const
{
  return m_data->m_paper;
}

//! operator<< for a PrintInfo
std::ostream &operator<< (std::ostream &o, PrintInfo const & r)
{
...
bool PrintInfo::read(TMWAWInputStreamPtr input)
{
