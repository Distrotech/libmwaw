...
MWAWPresentationListener::MWAWPresentationListener(MWAWParserState &parserState, std::vector<MWAWPageSpan> const &pageList, librevenge::RVNGPresentationInterface *documentInterface) : MWAWListener(),
  m_ds(new MWAWPresentationListenerInternal::GraphicState(pageList)), m_ps(new MWAWPresentationListenerInternal::State),
...
void MWAWPresentationListener::insertCharacter(unsigned char c)
{
  ...
  if (unicode == -1) {
    if (c < 0x20) {
      ...
      MWAWPresentationListener::insertChar((uint8_t) c);
  }
  else
    MWAWPresentationListener::insertUnicode((uint32_t) unicode);
}
...
void MWAWPresentationListener::insertField(MWAWField const &field)
{
  ...
  case MWAWField::Database:
    if (field.m_data.length())
      MWAWPresentationListener::insertUnicodeString(field.m_data.c_str());
    else
      MWAWPresentationListener::insertUnicodeString("#DATAFIELD#");
    break;
  case MWAWField::Date:
  case MWAWField::Time: {
    ...
    if (localtime_r(&now, &timeinfo)) {
      ...
      MWAWPresentationListener::insertUnicodeString(librevenge::RVNGString(buf));
    }
    break;
  }
  default:
...
// document
///////////////////
void MWAWPresentationListener::setDocumentLanguage(std::string locale)
{
...
bool MWAWPresentationListener::insertHeader(MWAWSubDocumentPtr subDocument, librevenge::RVNGPropertyList const &extras)
{
...
bool MWAWPresentationListener::insertFooter(MWAWSubDocumentPtr subDocument, librevenge::RVNGPropertyList const &extras)
{
  if (m_ds->m_isHeaderFooterStarted) {
...
void MWAWPresentationListener::insertPicture
(MWAWPosition const &pos, MWAWGraphicShape const &shape, MWAWGraphicStyle const &style)
{
  ...
  case MWAWGraphicShape::C_Bad:
    break;
  default:
    MWAW_DEBUG_MSG(("MWAWPresentationListener::insertPicture: unexpected shape\n"));
    break;
  }
}
...
(MWAWPosition const &pos, MWAWTable &table, MWAWGraphicStyle const &style)
{
  ...
  shared_ptr<MWAWListener> listen(this, MWAW_shared_ptr_noop_deleter<MWAWPresentationListener>());
  try {
...
void MWAWPresentationListener::addEmptyTableCell(Vec2i const &pos, Vec2i span)
{
  if (!m_ps->m_isTableRowOpened) {
...
// subdocument
///////////////////
void MWAWPresentationListener::handleSubDocument(Vec2f const &orig, MWAWSubDocumentPtr subDocument, libmwaw::SubDocumentType subDocumentType)
{
  if (!m_ds->m_isDocumentStarted) {
  ...
  if (sendDoc) {
    if (subDocument) {
      ...
      try {
        ...
        MWAW_DEBUG_MSG(("Works: MWAWPresentationListener::handleSubDocument exception catched \n"));
      }
...
// ---------- state stack ------------------
shared_ptr<MWAWPresentationListenerInternal::State> MWAWPresentationListener::_pushParsingState()
{
