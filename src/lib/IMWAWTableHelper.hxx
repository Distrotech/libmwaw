...
#ifndef IMWAW_TABLE_HELPER
#  define IMWAW_TABLE_HELPER

class IMWAWTableHelper;

/** a virtual structure used to store/send a cell to a listener */
class IMWAWTableHelperCell
{
  friend class IMWAWTableHelper;
public:
  //! constructor
  IMWAWTableHelperCell() : m_box(), m_position(-1,-1), m_numSpan() {
  }
  //! destructor
  virtual ~IMWAWTableHelperCell() { }
  ...
  //! operator<<
  friend std::ostream &operator<<(std::ostream &o, IMWAWTableHelperCell const &cell) {
  ...
  virtual bool send(IMWAWContentListenerPtr listener) = 0;

  //! call when the content of a cell must be send
  virtual bool sendContent(IMWAWContentListenerPtr listener) = 0;
...
protected:
  ...
  struct Compare {
    ...
    struct Point {
      Point(int wh, IMWAWTableHelperCell const *cell) : m_which(wh), m_cell(cell) {}
      ...
      int m_which;
      IMWAWTableHelperCell const *m_cell;
    };
...
public:
  //! the constructor
  IMWAWTableHelper() : m_cellsList(), m_rowsSize(), m_colsSize() {}

  //! the destructor
  virtual ~IMWAWTableHelper();

  //! add a new cells
  void add(shared_ptr<IMWAWTableHelperCell> cell) {
  ...
  shared_ptr<IMWAWTableHelperCell> get(int id);

  /** try to send the table

  Note: either send the table ( and returns true ) or do nothing.
   */
  bool sendTable(IMWAWContentListenerPtr listener);

  /** try to send the table as basic text */
  bool sendAsText(IMWAWContentListenerPtr listener);
...
protected:
  ...
  /** the list of cells */
  std::vector<shared_ptr<IMWAWTableHelperCell> > m_cellsList;
